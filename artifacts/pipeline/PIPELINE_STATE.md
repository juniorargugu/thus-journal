...
**Local HEAD:** `9a07fdc` (fix: reset G2 group map on hydration changes)
**Remote:** `origin/main` == `71283c3` (v3.22.0, deployed). **Local ahead by 4 unpushed commits** — 2 code (`ba9e780`, `9a07fdc` = G2 v0.4 loader/render + stale-map reset) + 2 docs (`a555414`, `f45416e`). **NOT deployed.**
**Deploy posture:** **PROD UNCHANGED** at `71283c3` / v3.22.0 (batch `f5290f7..b1f8e7d` + hotfix `71283c3` live, byte-identical). G2 v0.4 loader/render is **implemented + reviewed locally (Codex PASS → OK_FOR_FUTURE_BATCH)** but **unpushed**. G2 flags remain default-off; write gate NOT enabled.
...
| **B** | G2 grouping | v0.4 group-aware loader/render **implemented + Codex PASS locally** (`ba9e780`+`9a07fdc`, OK_FOR_FUTURE_BATCH), **unpushed**. Earlier write-gate browser smoke **PASS WITH ROLLBACK**; DB safe (0 active / 0 grouped, 1 archived); flags default-off | Future deploy-batch **preflight** (local stack, non-write validation) | push/deploy = user approval; flag enable = user approval |
