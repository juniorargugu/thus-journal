...
**Local HEAD:** `85e5116` (fix: clarify MT5 product preview labels)
**Remote:** local `main` ahead of `origin/main` by 7 (G2 stack + pipeline docs + MT5 preview cleanup), unpushed.
...
| **C** | Product/MT5 preview UX cleanup | **DONE** for MT5 Inbox Mapping preview (`85e5116`); trade-open picker deferred | Trade-open picker contract-size audit/design (separate review) | scope review before code |
...
- **C — Product/MT5 preview UX cleanup.** **DONE** for the MT5 Inbox Mapping preview surface (`85e5116`, 2026-07-07): `_mt5PreviewLabel`/`_mt5ContractPair` now render family-level `Name — base · Current <cur> · Next <nxt> · csize <n>` with the `matches row (current|next)` suffix preserved; display-string helpers only, no product_id/registry/persistence change; esbuild EXIT 0, 16/16 unit. **Deferred (separate review):** the trade-open form's per-series product picker (`expandProducts` cards + edit dropdown) still does NOT show contract size. Deferred because that surface drives the **persisted `productId`** — not the same task; needs its own audit/design + review before any code. Do not fold it into the MT5-preview work.
