...
HARD GUARANTEES
    ...
    - NO Journal trades / trade_groups / capture_events / Telegram / scheduler / S1.1
      (no account balance, equity or currency is captured or sent).
...
CONNECTOR_VERSION_DEFAULT = "s1-oneshot/0.1"
...
def print_preview(envelope, missing_by_pid, symbols_meta, account_facts, *, envelope_path, sha256):
    ...
    print(f"      python ops/mt5_import/s1_snapshot.py --write --confirm {CONFIRM_WRITE} \\")
    print(f"        --envelope {envelope_path} \\")
...
def parse_args(argv):
    ...
    ap.add_argument("--connector-version", dest="connector_version", default=CONNECTOR_VERSION_DEFAULT)
